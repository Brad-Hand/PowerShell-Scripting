...
$Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri "http://osla02.osla.org/PowerShell" -Authentication Kerberos

Import-PSSession $Session -CommandName *Mailbox

# Store the data from NewUsersFinal.csv in the $ADUsers variable
$ADUsers = Import-csv "\\osla09\is\PowerShell Scripts\AD Add Users\AddUsers.csv" -delimiter ','
...
foreach ($User in $ADUsers) {
    ...
    if (Get-ADUser -F { SamAccountName -eq $username }) {
        ...
        New-ADUser `
            ...
            -UserPrincipalName "$username@osla.org" `
            -Name $fullname `
            ...
            -HomeDrive "H:" `
            -HomeDirectory "\\osla04\users\$username" `
            -Manager $Manager `
            -Title $JobTitle `
...
foreach ($User in $ADUsers) {
    ...
    If (-not $Mailbox) {
       ...
       #Create Exchange Mailbox
       Enable-RemoteMailbox -Identity $username -RemoteRoutingAddress "$username@NFPOSLA.mail.onmicrosoft.com" | Out-Null
